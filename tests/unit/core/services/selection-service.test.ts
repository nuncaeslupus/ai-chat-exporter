...
describe('SelectionService', () => {
  ...
  describe('selectRange()', () => {
    it('selects all pairs in range inclusive', () => {
      const result = SelectionService.selectRange(pairs, 0, 2);
      expect(result[0]!.selected).toBe(true);
      expect(result[1]!.selected).toBe(true);
      expect(result[2]!.selected).toBe(true);
    });
    ...
    it('selects partial range', () => {
      const morePairs = [
        ...pairs,
        createTestQAPair(3, 'Q4', 'A4', false),
        createTestQAPair(4, 'Q5', 'A5', false),
      ];
      const result = SelectionService.selectRange(morePairs, 1, 3);
      expect(result[0]!.selected).toBe(true);
      expect(result[1]!.selected).toBe(true);
      expect(result[2]!.selected).toBe(true);
      expect(result[3]!.selected).toBe(true);
      expect(result[4]!.selected).toBe(false);
    });

    it('handles reversed range (end < start)', () => {
      const result = SelectionService.selectRange(pairs, 2, 0);
      expect(result[0]!.selected).toBe(true);
      expect(result[1]!.selected).toBe(true);
      expect(result[2]!.selected).toBe(true);
    });

    it('handles single index range', () => {
      const result = SelectionService.selectRange(pairs, 1, 1);
      expect(result[0]!.selected).toBe(true);
      expect(result[1]!.selected).toBe(true);
      expect(result[2]!.selected).toBe(false);
    });
    ...
    it('handles out of bounds indices gracefully', () => {
      const result = SelectionService.selectRange(pairs, 1, 10);
      expect(result[0]!.selected).toBe(true);
      expect(result[1]!.selected).toBe(true);
      expect(result[2]!.selected).toBe(true);
    });
    ...
    it('does not mutate the original array', () => {
      const original = [...pairs];
      SelectionService.selectRange(pairs, 0, 2);
      expect(pairs[2]!.selected).toBe(original[2]!.selected);
    });
  });

  describe('invertSelection()', () => {
    it('inverts all selections', () => {
      const result = SelectionService.invertSelection(pairs);
      expect(result[0]!.selected).toBe(false);
      expect(result[1]!.selected).toBe(false);
      expect(result[2]!.selected).toBe(true);
    });

    it('inverts when all are selected', () => {
      const allSelected = pairs.map((p) => ({ ...p, selected: true }));
      const result = SelectionService.invertSelection(allSelected);
      expect(result[0]!.selected).toBe(false);
      expect(result[1]!.selected).toBe(false);
      expect(result[2]!.selected).toBe(false);
    });

    it('inverts when none are selected', () => {
      const noneSelected = pairs.map((p) => ({ ...p, selected: false }));
      const result = SelectionService.invertSelection(noneSelected);
      expect(result[0]!.selected).toBe(true);
      expect(result[1]!.selected).toBe(true);
      expect(result[2]!.selected).toBe(true);
    });
    ...
    it('does not mutate the original array', () => {
      const original = [...pairs];
      SelectionService.invertSelection(pairs);
      expect(pairs).toEqual(original);
    });

    it('handles empty array', () => {
      const result = SelectionService.invertSelection([]);
      expect(result).toEqual([]);
    });
  });
});
