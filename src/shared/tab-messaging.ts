...
/**
 * Inject the content scripts the manifest declares. The manifest is the single
 * source of the file list, so a renamed bundle needs no second edit here.
 *
 * ponytail: injects every `content_scripts` entry; there is exactly one. Filter
 * on `matches` if a narrower second entry ever lands.
 */
