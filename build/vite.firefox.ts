...
function firefoxManifestPlugin() {
  return {
    name: 'firefox-manifest-plugin',
    closeBundle() {
      const baseManifest = JSON.parse(
        readFileSync(resolve(rootDir, 'manifests/manifest.base.json'), 'utf-8')
      );
      const firefoxManifest = JSON.parse(
        readFileSync(resolve(rootDir, 'manifests/manifest.firefox.json'), 'utf-8')
      );

      const merged = deepMerge(baseManifest, firefoxManifest);
